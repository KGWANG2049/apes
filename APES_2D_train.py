...
if __name__ == '__main__':
    ...
    for i in range(0, BUFFER_MAX):
        ...
        para_diri = gen_model(OC, SV, GV)
        dirichlet_dist = Dirichlet(para_diri)
        W = dirichlet_dist.rsample()
        # print("W", W, W.shape)
    ...
    for i in range(5):
        ...
        for k in range(REPLAY_SAMPLE_SIZE):
            (mean, std) = critic_model(sampled_oc[k], sampled_start_v[k], sampled_goal_v[k],
                                       sampled_coefficients[k])
            # dir_dist = Dirichlet(sampled_coefficients[k])
            # entropy = gen_model(sampled_oc[gen_idx], sampled_start_v[gen_idx], sampled_goal_v[gen_idx]).entropy()
...
"""
        ...
        for m in range(REPLAY_SAMPLE_SIZE):
            ...
            entropy = gen_model(sampled_oc[m], sampled_start_v[m], sampled_goal_v[m]).entropy()
            print("entropy", entropy)
...
"""
    ...
    # modified can be saved 08.03,23 RuntimeError: one of the variables needed for gradient computation has been
    # modified by an inplace operation: [torch.cuda.DoubleTensor [200, 50]], which is output 0 of AsStridedBackward0,
    # is at version 2; expected version 1 instead. Hint: enable anomaly detection to find the operation that failed
